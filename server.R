...
shinyServer(function(input, output,session){
  ...
  output$plot <- renderPlot({
    sdata<-score()
    d1000<-read.table("~/data/1000Genome_cervical cancer.txt",head=T,sep="\t",stringsAsFactors = F)
    ...
    text(dim(d1000)[1]/2,ymax+70,"Density Curve of Population Genetic Risk",cex=1.75)
...
score.score()
  ...
  output$range <- renderText({
    calc.range <- reactive({
      ...
      selectdata<-sdata[1,c("Sum(OR)")]
      if(!is.null(selectdata)){
        if(selectdata==0){print("VCF文件未包含宫颈癌相关位点")}else{
        d1000<-read.table("~/data/1000Genome_cervical cancer.txt",header=TRUE,sep="\t",stringsAsFactors = F)
...
version3<-function(tabdata){
  version3database<-read.table("~/data/cervical cancer.txt",head=T,sep="\t",stringsAsFactors = F)
  cg<-c("C","G")
  at<-c("A","T")
...
vcf2tab <- function(vcf){
  ...
  GT1 <- substr(info,1,3)
  GT2 <- ""
  database4<-read.table("~/data/cervical cancer.txt",head=T,sep="\t",stringsAsFactors = F)
  DP <- ""
  AD <- ""
  PL <- ""
  dbSNP <- ""
